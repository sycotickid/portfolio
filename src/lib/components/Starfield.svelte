<script lang="ts">
	...
	onMount(() => {
		...
		type Star = { x: number; y: number; size: number; opacity: number; phase: number; speed: number; flicker: boolean };
		const stars: Star[] = [];
		...
		function scheduleShooter() {
			const delay = SHOOTER_INTERVAL_MIN + Math.random() * SHOOTER_INTERVAL_SPREAD;
		...
		function drawShooter() {
			...
			const grad = ctx.createLinearGradient(s.x, s.y, s.x - nx * s.length, s.y - ny * s.length);
			grad.addColorStop(0,   `rgba(255, 255, 255, ${s.alpha.toFixed(3)})`);
			grad.addColorStop(0.3, `rgba(180, 210, 255, ${(s.alpha * 0.6).toFixed(3)})`);
			...
			if (s.alpha <= 0 || s.x > canvas.width + s.length || s.y > canvas.height + s.length) {
				shooter = null;
			}
		}
		...
		function draw() {
			...
			for (const s of stars) {
				const x = ((s.x - mouseOffsetX) % canvas.width  + canvas.width)  % canvas.width;
				const y = ((s.y - scrollOffset - mouseOffsetY) % canvas.height + canvas.height) % canvas.height;
